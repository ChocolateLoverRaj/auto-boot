...
// For some reason a size of 512 bytes maximizes the read speed to ~945KiB/s, even though on 64 bytes are transferred at a time
const ZERO_BUFFER_SIZE: usize = 512;
const ZERO_BUFFER: [u8; ZERO_BUFFER_SIZE] = [0; ZERO_BUFFER_SIZE];
...
const BLOCKS: u32 = 2147483648; // Total data: exactly 1TiB
    ...
    match command.kind {
        ...
        ScsiCommand::Read { lba, len } => unsafe {
            ...
            if STATE.storage_offset != (len * BLOCK_SIZE) as usize {
                ...
                let transfer_size = (end - start).min(ZERO_BUFFER_SIZE);
                defmt::info!("Data transfer >>>>>>>> [{}..{}]", start, end);
                let count = command.write_data(&ZERO_BUFFER[..transfer_size])?;
